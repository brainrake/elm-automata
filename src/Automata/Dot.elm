module Automata.Dot exposing (..)

{-|


# NOTE: This was adapted from <https://github.com/elm-community/graph/blob/6.0.0/src/Graph/DOT.elm>

This module provides a means of converting the `FSA` data type into a
...
-}
output : (n -> Maybe String) -> (e -> Maybe String) -> NFA n e -> String
output =
...
-}
outputWithStyles : Styles -> (n -> Maybe String) -> (e -> Maybe String) -> NFA n e -> String
...
outputWithStylesAndAttributes styles nodeAttrs edgeAttrs graph =
    let
        ...
        edgesString =
            List.map edge graph.transitions
                |> String.join "\n"

        edge e =
            "  "
                ++ getLabel (nodeAttrs e.from)
                ++ " -> "
                ++ getLabel (nodeAttrs e.to)
                ++ makeAttrs (edgeAttrs e.symbol)
        ...
        node n =
            "  "
                ++ getLabel (nodeAttrs n)
                ++ makeAttrs (nodeAttrs n)
